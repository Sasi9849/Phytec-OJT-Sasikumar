...
/**
  ...
  */
  ...
  /* USER CODE BEGIN 1 */
//float Analog;
void mqtt_data_send(float value)
{
char buffer[50];

sprintf (&buffer[0], "CMD+MQTTPUB=sensor/voltage,%.2f\r\n",value);
...
int main(void)
{
  ...
  MX_USART1_UART_Init();
  /* USER CODE BEGIN 2 */
  WE10_Init("realme X7 Max","0987654321");
  MQTT_Init();
  /* USER CODE END 2 */
  ...
  while (1)
	  ...
	  float Analog;
	    ...
	    {
	      Analog = HAL_ADC_GetValue(&hadc1) * (5.0 / 1023.0);
	    }
	    HAL_ADC_Stop(&hadc1);

	    // Read the digital value from the specified pin
	    Digital = HAL_GPIO_ReadPin(GPIOB, GPIO_PIN_0);  // Change GPIO_PIN_0 to your specific pin

	    // Output analog and digital values over UART
	    float thresholdTemperature = 2.5; // Example threshold temperature in volts

	       if (Analog > thresholdTemperature)
	       {
	         Digital = 1;
	       }
	       else
	    ...
	    snprintf(uart_buffer, sizeof(uart_buffer), "Analog voltage value: %.4f V, %s \r\n", Analog, Digital == 1 ? "reached" : "not yet reached");
	    HAL_UART_Transmit(&huart2, (uint8_t *)uart_buffer, strlen(uart_buffer), HAL_MAX_DELAY);
	    mqtt_data_send(Analog);
	    HAL_Delay(200);
	  }
}
    /* USER CODE END WHILE */
    ...
    /* USER CODE BEGIN 3 */
	 void WE10_Init (char *SSID, char *PASSWD)
	  ...
	  void MQTT_Init()
	  {
	  	...
	  	/*********CMD+MQTTCONCFG---->LED ********/
	  	//memset(&buffer[0],0x00,strlen(buffer));
	  	sprintf (&buffer[0], "CMD+MQTTCONCFG=3,mqtt-sasikumar63092-ow01tb,,,,,,,,,\r\n");
	  	HAL_UART_Transmit(&huart1, (uint8_t*)buffer, strlen(buffer), 1000);
	  	HAL_UART_Transmit(&huart2, (uint8_t*)buffer, strlen(buffer), 1000);
	  	//memset(&buffer[0],0x00,strlen(buffer));
	  	//HAL_Delay(500);
