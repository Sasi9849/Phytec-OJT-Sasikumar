...
osThreadId SendTaskhandleHandle;
osThreadId RecieveTaskHandHandle;
...
void RecieveTask(void const * argument);
...
int main(void)
{
  ...
  /* Create the thread(s) */
  /* definition and creation of SendTaskhandle */
  osThreadDef(SendTaskhandle, SendTask, osPriorityNormal, 0, 128);
  SendTaskhandleHandle = osThreadCreate(osThread(SendTaskhandle), NULL);

  /* definition and creation of RecieveTaskHand */
  osThreadDef(RecieveTaskHand, RecieveTask, osPriorityIdle, 0, 128);
  RecieveTaskHandHandle = osThreadCreate(osThread(RecieveTaskHand), NULL);
...
/**
  * @brief  Function implementing the SendTaskhandle thread.
...
void SendTask(void const * argument)
{
  ...
  /* Infinite loop */
	...
	osDelay(1000); // Delay for 1000 milliseconds (1 second) before thenext iteration
	}
  /* USER CODE END 5 */
}

/* USER CODE BEGIN Header_RecieveTask */
/**
* @brief Function implementing the RecieveTaskHand thread.
* @param argument: Not used
* @retval None
*/
/* USER CODE END Header_RecieveTask */
void RecieveTask(void const * argument)
{
  /* USER CODE BEGIN RecieveTask */
  /* Infinite loop */
	data retvalue;
	for(;;)
	{
	...
	char buffer[128];
	sprintf (&buffer[0], "CMD+MQTTPUB=base/state/temperature,%.2f\r\
	n",retvalue.humi);
